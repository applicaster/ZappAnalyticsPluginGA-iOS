...
@objc open class GAProvider: ZPAnalyticsProvider {
    ...
    override open func trackEvent(_ eventName: String, parameters: [String: NSObject]) {
        ...
        let convertedCustomDimensions = self.convertedCustomDimensions(forInitialParams: self.combinedInitialParams, eventParams: parameters)
    ...
    fileprivate func convertedCustomDimensions(forInitialParams initialParams: GAProviderParams, eventParams: GAProviderParams) -> GAProviderParams {
        var result = GAProviderParams()

        let remoteParams = ZPAnalyticsProvider.getFirebaseRemoteConfigurationParameters(prefix: Consts.customDimentionsPrefix, baseProperties: initialParams, eventProperties: eventParams)
